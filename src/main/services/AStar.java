...
public class AStar implements CustomAlgorithm
{
...
//						// H-Cost für den Knoten auf das aktuelle hLevel setzen
						_adjacencyMatrix.get(itNode.getLabel()).set_hCost(hLevel*2);
//						
...
//					
	...
	public void iterate()
	{
		...
		if( _queue.size() > 0 )
		{
					...
					// Wenn der Knoten noch nie erkundet wurde
							...
							)
						{
							...
							ausgabe += "\n G-Cost: " + adjPredecessor.getLabel() + " + " + adjVertex.getLabel() + ": "
									...
									+ " | F-Cost " + adjVertex.getLabel() + ": " + getFValue(adjVertex);
			...
			{				
				...
				if( getFValue(_graph.getVertex(line.getVertex2().getLabel()))
						< getFValue(_graph.getVertex(_bestLine.getVertex2().getLabel()))
						&& !_closedList.contains(_graph.getVertex(line.getVertex2().getLabel())) )
				{
					_bestLine = line;
					_bestNode = _graph.getVertex(_bestLine.getVertex2().getLabel());
	...
	private int getFValue(GraphLine edge)
	{
		...
		System.out.println("---------> " + vertex2.getLabel() + "(" + getGValue(vertex2)
		+ ", " + getHValue(vertex2) + ")");
		return (getGValue(vertex2) + getHValue(vertex2));
	}
	
	// accumulated movement costs (G+H)
	private int getFValue(GraphVertex vertex)
	{	
		System.out.println("---------> " + vertex.getLabel() + "(" + getGValue(vertex)
				+ ", " + getHValue(vertex) + ")");
		return getGValue(vertex) + getHValue(vertex);
	}
	...
	private void runGoalFound()
	{
//		ArrayList<GraphVertex> shortestPath = new ArrayList<GraphVertex>();
//		CopyOnWriteArrayList<GraphVertex> merkliste = new CopyOnWriteArrayList<GraphVertex>();
//		CopyOnWriteArrayList<GraphVertex> queue = new CopyOnWriteArrayList<GraphVertex>();
//		
//		shortestPath.add(_goal);
//		queue.add(_goal);
//		
//		while( queue.size() > 0 && !queue.get(0).getLabel().equals("vS") )
//		{
//			GraphVertex predecessor = _adjacencyMatrix.get(queue.remove(0).getLabel()).getPredecessor();
//			shortestPath.add(0, predecessor);
//			queue.add(_adjacencyMatrix.get(predecessor.getLabel()).getPredecessor());
//		}
//		
//		String ausgabe = "vS ";
