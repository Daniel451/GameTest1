...
public class GraphDirected implements Graph
{
	...
	public GraphDirected(ArrayList<GraphVertex> vertexes, ArrayList<GraphLine> edges)
	{
		_vertexes = vertexes;
		_edges = edges;
	...
	private void createAdjacencyListOutgoing()
	{
		...
		for(GraphLine line : _edges)
		{
			...
			// Wenn die Adjazenzliste den ersten Knoten der aktuellen Kante bereits enthält
			if(_adjacencyListOutgoing.containsKey(currentVertex.getLabel()))
			{
				ArrayList<GraphLine> containerList = _adjacencyListOutgoing.get(currentVertex.getLabel());
				
				// Der Adjazenzliste die aktuelle Kante hinzufügen, sofern sie noch nicht enthalten ist
			...
			// Wenn es zu diesem Knoten noch keinen Eintrag gibt, dann eine neue Adjazenzliste erstellen
		...
		// Falls irgendwelche Knoten keine ausgehenden Kanten besitzen, leere Adjazenzlisten hinzufügen
		for(GraphVertex vertex : _vertexes)
		{
			...
			{
				_adjacencyListOutgoing.put(vertex.getLabel(), new ArrayList<GraphLine>());
			}
		}
	}
	...
	private void createAdjacencyListIncoming()
	{
		...
		for(GraphLine line : _edges)
		{
			...
			{
				ArrayList<GraphLine> containerList = _adjacencyListIncoming.get(currentVertex.getLabel());
		...
		for(GraphVertex vertex : _vertexes)
		{
			...
			{
				_adjacencyListIncoming.put(vertex.getLabel(), new ArrayList<GraphLine>());
			}
		}
	}
   ...
   public boolean containsVertex(String vertex)
   {
	...
	/**
	 * Gibt anhand eines Strings (Label des Knotens) den entsprechenden Knoten zurück, sofern er enthalten ist.
	 * Ansonsten wird eine Nullreferenz zurückgegeben.
	 * 
	 * @return GraphVertex Ein valides Knotenobjekt, dass dem übergebenen String entspricht
	 */
	@Override
   ...
   {
	   ArrayList<GraphLine> list1 = _adjacencyListOutgoing.get(vertex.getLabel());
	   ArrayList<GraphLine> list2 = _adjacencyListIncoming.get(vertex.getLabel());
	   
	   ArrayList<GraphVertex> retList = new ArrayList<GraphVertex>();
	   
	   // Ausgehende Kanten checken und Vertex2 der Nachbarliste hinzufügen, falls noch nicht vorhanden
	   ...
	   // Eingehende Kanten checken und Vertex1 der Nachbarliste hinzufügen, falls noch nicht vorhanden
   ...
   public ArrayList<GraphLine> getIncomingEdgesOfVertex(GraphVertex vertex)
   {
	...
	public void printAllData()
	{
		...
		System.out.println("-------------------");
		System.out.println("Vertexes" + " ( " + _vertexes.size() + " Vertex(es) in Graph )");
		System.out.println("-------------------");
		
		for( int i = 0; i < _vertexes.size(); i++ )
		{
			System.out.println("_vertexes.get(" + i + "): " + "'" + _vertexes.get(i).getLabel() + "'" );
		}
		
		System.out.println("-------------------");
		System.out.println("Edges" + " ( " + _edges.size() + " Edge(s) in Graph )");
		System.out.println("-------------------");
		
		for( int i = 0; i < _edges.size(); i++ )
		{
			System.out.println("_edges.get(" + i + "): " + "'" + _edges.get(i).getLabel() + "'" );
		}
		...
		{
			System.out.println("[" + vertexLabel + "] -> '" + _adjacencyListOutgoing.get(vertexLabel).toString() + "'");
		}
		...
		{
			System.out.println("[" + vertexLabel + "] -> '" + _adjacencyListIncoming.get(vertexLabel).toString() + "'");
		}
	}
