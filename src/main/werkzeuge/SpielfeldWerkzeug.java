...
public class SpielfeldWerkzeug extends Observable
{
	...
	// Für Zeichenvorgänge
		...
		HashMap<String, GraphVertexUI> _vertexUImap = new HashMap<String, GraphVertexUI>();
	...
	private void zeichneKnoten()
	{
		...
		// Zeichnen
			...
			for(GraphVertex knoten : _graph.getAllVertexes())
			{
				...
				if( _graph.getOutgoingEdgesOfVertex(knoten).size() >= 1 && !startVertexIsAlreadyDrawn)
				{
					GraphVertexUI neuerUIknoten = new GraphVertexUI(x_a, y_a, knoten.getLabel());
			...
			// Alle direkten Nachfolger von dem Startknoten und seinen transitiven Nachfolgern zeichnen
	 ...
	 * 
	 * @param vertex Aktueller Knoten, vom die Nachfolger gezeichnet werden sollen
	 * @param x_a Aktueller X-Wert
	 * @param y_a Aktueller Y-Wert
	 * @param level Aktuelle Tiefe des Baumes
	 */
	private void paintSuccessorVertexes(GraphVertex vertex, int x_a, int y_a, int level)
	{
		// Vorbereitungen
		...
		if( liste.size() >= 1 )
		{
			...
			{
				GraphVertex itVertex = _graph.getVertex(liste.get(i).getVertex2().getLabel());
				...
				if( !_drawnVertexes.contains(liste.get(i).getVertex2()) )
				{
					...
					// davon die Hälfte der gesamten neuen Höhe (Anzahl neue Knoten * y_inc / 2)
					// abgezogen und am Ende für jeden Iterationsschritt wieder y_inc * i hinzuaddiert
					int y_neu = ( y_a - (y_inc * (liste.size()-1) / 2) ) + y_inc * i; 
					
					GraphVertexUI neuerUIknoten = new GraphVertexUI( x_neu, y_neu, itVertex.getLabel() );
					neuerUIknoten.changeColor(itVertex.getColor());
					_spielfeld.addComponent(neuerUIknoten);
					_vertexUImap.put(itVertex.getLabel(), neuerUIknoten);
					_drawnVertexes.add(itVertex);
					
					// rekursiver Aufruf, um die Kinder des aktuellen Knotens zu zeichnen
					paintSuccessorVertexes( itVertex, x_a, neuerUIknoten.getY(), level+1 );
				}
			}
		}
	}
	...
	private void zeichneKanten()
	{
		...
		for( GraphLine kante : kantenliste )
		{
			...
			GraphVertexUI ausgehenderKnotenUI = _vertexUImap.get(ausgehenderKnoten.getLabel());
			GraphVertexUI eingehenderKnotenUI = _vertexUImap.get(eingehenderKnoten.getLabel());
			
			Punkt p_start = ausgehenderKnotenUI.gibZentrum();
			Punkt p_end = eingehenderKnotenUI.gibZentrum();
			
			_spielfeld.addComponent(new GraphLineUI(
					p_start.getIntX(),
					p_start.getIntY(),
					p_end.getIntX(),
					p_end.getIntY(),
	...
	private void erstelleButtonListener()
	{
		...
		_spielfeld.getButton("button_template").addActionListener(
                  ...
                  public void actionPerformed(ActionEvent e)
                  {
							...
							{
								setChanged();
								ObservedData data = new ObservedData("TemplateSelected", chooser.getSelectedFile());
		...
		_spielfeld.getButton("button_start").addActionListener(
               ...
               {
						setChanged();
						ObservedData data = new ObservedData("StartButton", _spielfeld.getRadioButtonGroup().getSelection());
						notifyObservers(data);
               }
		...
		_spielfeld.getButton("button_next").addActionListener(
               ...
               {
						setChanged();
						ObservedData data = new ObservedData("NextButton", _spielfeld.getRadioButtonGroup().getSelection());
						notifyObservers(data);
               }
